...
def file_selector() -> Tuple[str, ...]:
    ...
    创建隐藏的Tkinter根窗口，弹出标准文件选择对话框供用户选择多个文件。
    支持所有文件类型的多选操作，用户可以通过Ctrl/Cmd+点击选择多个文件。
    
    技术实现：
    - 创建临时Tkinter根窗口并隐藏，避免显示空白窗口
    - 使用系统原生文件对话框，提供最佳用户体验
    - 支持键盘快捷键和鼠标操作的多文件选择
    ...
    Note:
        - 该函数会自动创建和销毁Tkinter窗口，不会影响主应用程序
        - 支持所有操作系统的原生文件选择界面
        - 返回的路径都是绝对路径，便于后续处理
    """
    # 创建Tkinter根窗口并隐藏，避免显示空白窗口
    root = tk.Tk()
    root.withdraw()
