...
from PySide6.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, 
                             QWidget, QPushButton, QLabel, QScrollArea, QLineEdit, QSplitter, QGroupBox,
...
from PySide6.QtGui import QColor, QPalette, QCursor, QAction, QMouseEvent
...
class TemplateButton(QPushButton):
    ...
    def set_variable_info(self, description: str, gui_info: Optional[Dict[str, Any]] = None):
        ...
        print(f"[gui.py] TemplateButton.set_variable_info for '{variable_name}': gui_info is {self.variable_gui_info}")
...
class TemplateEditor(QWidget):
    ...
    def add_variable_to_template(self, variable_name: str):
        """
        向模板中添加一个变量按钮。
        通常由左侧变量列表的点击事件触发。
        """
        # 1. 创建变量按钮
        variable_content = f"{{{variable_name}}}"
        button = TemplateButton(variable_content, "variable", parent_editor=self)
        description, gui_info = self.get_variable_info(variable_name)
        button.set_variable_info(description, gui_info)

        # 2. 找到当前活动的文本输入框
        active_text_edit_wrapper = None
        for node in self.component_list:
            if node.component_type == 'text' and node.widget_instance.text_edit.hasFocus():
                active_text_edit_wrapper = node.widget_instance
                break
        
        # 如果没有活动的输入框，则添加到最后一个文本输入框
        if not active_text_edit_wrapper:
            for node in reversed(self.component_list):
                 if node.component_type == 'text':
                    active_text_edit_wrapper = node.widget_instance
                    break

        # 3. 将变量插入
        if active_text_edit_wrapper:
            current_node = self.component_list.find_node_by_widget(active_text_edit_wrapper)
            if current_node:
                # 在当前节点之前插入
                self.component_list.insert_before(current_node, 'variable', button)
        else:
            # 如果没有任何文本框，则直接添加
    ...
    def get_variable_info(self, variable_name: str) -> tuple[str, Optional[Dict[str, Any]]]:
        """获取变量的描述和GUI信息"""
...
class FileClassifierGUI(QMainWindow):
    ...
    def create_variables_panel(self, parent):
        ...
        """)
        separator_btn.setFixedWidth(350)
    ...
    def load_variables(self, layout):
        ...
        try:
            ...
            for plugin_info in variables_info:
                ...
                """)
                group_box.setFixedWidth(350)  # 设置固定宽度
