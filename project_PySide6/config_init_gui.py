...
class ConfigInitDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # 输出目录设置组
        plugin_group = QGroupBox("插件目录设置")
        plugin_layout = QVBoxLayout(plugin_group)

        # 输出目录说明
        plugin_desc = QLabel("选择插件存储目录：")
        ...
        self.plugin_dir_edit.setPlaceholderText("点击\"浏览\"选择目录...")
        self.plugin_dir_edit.setReadOnly(True)
        self.plugin_dir_edit.setStyleSheet("""
            QLineEdit {
                padding: 8px;
        ...
        """)
        plugin_row.addWidget(self.plugin_dir_edit)
        
        plugin_browse_btn = QPushButton("浏览...")
        plugin_browse_btn.setFixedWidth(80)
        plugin_browse_btn.clicked.connect(self.browse_plugin_directory)
        plugin_browse_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        plugin_row.addWidget(plugin_browse_btn)
    ...
    def browse_plugin_directory(self):
        """浏览选择输出目录"""
        current_dir = self.plugin_dir_edit.text() or os.path.expanduser("~")
        ...
        if selected_dir:
            self.plugin_dir_edit.setText(selected_dir)
    ...
    def accept_config(self):
        ...
        if not os.access(output_dir, os.W_OK):
            ...
            return
        # 验证插件目录
        plugin_dir = self.plugin_dir_edit.text().strip()
        if not plugin_dir:
            QMessageBox.warning(self, "错误", "请选择插件目录！")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"无法创建插件目录：{str(e)}")
            return
            
        # 检查目录是否可写
        if not os.access(plugin_dir, os.W_OK):
            QMessageBox.critical(self, "错误", "插件目录不可写，请选择其他目录！")
            return
        ...
        # 保存配置
        if self.save_config(output_dir, algorithm, plugin_dir):
    ...
    def save_config(self, output_dir: str, algorithm, plugin_dir: str) -> bool:
        """保存配置到配置文件"""
        try:
            # 导入配置模块
            ...
            # 创建配置实例
            config.Config(target_folder=output_dir, hash_check_enable=algorithm,external_plugins_dir=plugin_dir)
