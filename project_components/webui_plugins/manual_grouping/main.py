...
def clear_plugin_config() -> bool:
    ...
    try:
        ...
        # 保存空配置到文件
        with open(config_file, 'w', encoding='utf-8') as f:
            yaml.dump(empty_config, f, default_flow_style=False, allow_unicode=True, indent=2)
...
def update_plugin_groups(groups_data):
    """更新插件分组配置 - Web UI API"""
    try:
        global GLOBAL_STRATEGY_GROUPS
        GLOBAL_STRATEGY_GROUPS = []
        ...
        return {
            'success': True,
            'message': f'成功更新 {len(GLOBAL_STRATEGY_GROUPS)} 个分组配置'
        }
...
def test_file_matching(filename: str):
    """测试文件匹配 - Web UI API"""
    try:
        result = manual_grouping(filename)
        return {
            'success': True,
            'filename': filename,
            'matches': result
